...
func move(delta):
	...
	if velocity.x < 0:
		$MageSpiritDealDmgArea.scale.x = -1
	elif velocity.x > 0:
		$MageSpiritDealDmgArea.scale.x = 1
...
func _physics_process(delta: float) -> void:
	Global.MageSpiritDmgAmt = dmg_to_deal
	Global.MageSpiritDmgZone = $MageSpiritDealDmgArea
	...
	if Global.PlayerFullMoon && !isDead:
		...
		if !isDead && is_dealing_dmg:
			...
			if !isDead && !taking_damage:
				...
				$MageSpiritHitBox.queue_free()
				#$MageSpiritDealDmgArea.queue_free()
...
func _on_mage_spirit_hit_box_area_entered(area: Area2D) -> void:
...
func _on_mage_spirit_tracking_radius_body_entered(body: Node2D) -> void:
	if body == Global.PlayerBody:
		isChasing = true
	else:
		isChasing = false
